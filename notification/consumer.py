...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
		...
		try:
			if command == "get_general_notifications":
				...
				try:
					...
					pass
			# elif command == "get_available_chat":
			# 	user = self.scope['user']	
			# 	print(await get_recent_chatroom_messages(user))
			# 	payload = await get_recent_chatroom_messages(user)
				
			# 	if payload != None:
			# 		payload = json.dumps(payload.__dict__)
			# 		await self.send_recent_chatroom_message(payload)
	...
	async def send_recent_chatroom_message(self,payload):
		"""
		send the user friend info to to ui
		"""
		await self.send_json({
			"general_msg_type": 99,
			"messages_payload": "recent_chat_payload",
			"payload": payload
		})
